...
from scraper import Rom, get_available_systems, get_crc32_from_file, get_image_files_without_extension, get_roms, load_config_from_json, scrape_screenshot
...
def load_roms_menu() -> None:
	...
	if input.key("B"):
		...
		success: int = 0
		failure: int = 0
		gr.draw_log(f"Scraping {progress} of {len(roms_without_image)}", fill=gr.colorBlue, outline=gr.colorBlueD1)
		gr.draw_paint()
		for rom in roms_without_image:
			if rom.name not in imgs_files:
				rom.set_crc(get_crc32_from_file(system_path / rom.filename))
				screenshot: Optional[bytes] = scrape_screenshot(
					game_name=rom.name, crc=rom.crc, system_id=system_id
				)
				if screenshot:
					img_path: Path = imgs_folder / f"{rom.name}.png"
				...
				gr.draw_log(f"Scraping {progress} of {len(roms_without_image)}", fill=gr.colorBlue, outline=gr.colorBlueD1)
				gr.draw_paint()
		gr.draw_log(f"Scraping completed! Success: {success} Errors: {failure}", fill=gr.colorBlue, outline=gr.colorBlueD1, width=800)
		gr.draw_paint()
		time.sleep(4)
		exit_menu = True
	elif input.key("Y"):
		...
		if roms_selected_position < len(roms_list) - 1:
			roms_selected_position = min(len(roms_list) - 1, roms_selected_position + max_elem)
	elif input.key("L2"):
		if roms_selected_position > 0:
			roms_selected_position = max(0, roms_selected_position - 100)
	elif input.key("R2"):
		if roms_selected_position < len(roms_list) - 1:
			roms_selected_position = min(len(roms_list) - 1, roms_selected_position + 100)
	...
	for i, rom in enumerate(roms_without_image[start_idx:end_idx]):
		row_list(rom.name[:48] + "..." if len(rom.name) > 50 else rom.name, (20, 50 + (i * 35)), 600, i == (roms_selected_position % max_elem))
